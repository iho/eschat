...
handle_create_chat(Req = #{user_id := UserId, json_data := #{<<"name">> := Name}},
                   State) ->
...
handle_add_member(Req = #{chat_id := ChatId,
                          user_id := UserId,
...
handle_remove_member(Req = #{chat_id := ChatId,
                             user_id := UserId,
                             json_data := #{<<"user_id">> := MemberId}},
                     State) ->
    lager:debug("Removing member ~p from chat ~p~n", [MemberId, ChatId]),
...
handle_update_name(Req = #{chat_id := ChatId,
                           user_id := UserId,
...
handle_last_read_put(Req = #{chat_id := ChatId,
                             user_id := UserId,
